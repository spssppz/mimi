					...
					{FAQItems.map((item, index) => {
						...
						return (
							<div key={index} className="border-b border-[#d9d9d9] overflow-hidden py-3">
								<button
									className={`py-3 cursor-pointer w-full transition-colors duration-300 hover:text-foreground font-semibold ${isOpen ? 'text-foreground' : 'text-[#acacac]'} text-[18px] lg:text-[20px] text-left gap-2 flex justify-between items-center`}
									onClick={() => toggle(index)}
								>
									{item.title}
									<RightArrowIcon className={`w-6 h-6 lg:w-8 lg:h-8 transition-transform duration-300 ${isOpen ? 'rotate-270' : 'rotate-90'}`}></RightArrowIcon>
								</button>
