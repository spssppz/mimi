...
export default function Header() {
	...
	return (
		...
		>
			<div className="max-w-308 mx-auto px-4 flex items-center lg:justify-between gap-5">
				...
				<div className="flex gap-6 xl:gap-8 items-center order-3 lg:order-2">
					...
					<button
						...
						className={`lg:hidden relative z-12 ${enabled ? 'text-white' : 'text-brand-light-gray'} lg:text-[#777777] w-6 h-6 basis-6 cursor-pointer`}
					>
				...
				<a href="tel:+740122344555" className={`order-2 lg:order-3 rounded-full shadow-[inset_0_2px_4px_0_rgba(0,0,0,0.06)] p-2 border min-w-9.5 lg:min-w-auto min-h-9.5 lg:min-h-auto lg:px-4 lg:pt-2.5 lg:pb-2 flex items-center justify-center transition duration-400 gap-2 font-medium text-[13px] ${enabled ? 'text-white hover:bg-white hover:text-[#00d0ff] border-[#00d0ff]' : 'bg-white hover:bg-[#00d0ff] hover:text-white border-[#EFEFEF]'}`}>
			...
			{
				isOpen && (
					<div className={`${enabled ? "bg-foreground" : "bg-white"} fixed top-0 z-11 left-0 w-full overflow-auto h-full px-8 pt-20 pb-10 transitions-colors duration-300 font-helvetica`}>
